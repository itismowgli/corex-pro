...
### Added
- **`/etc/corex/backup.conf` is written, having been read by three scripts and
  created by none.** `corex-backup.sh`, `corex-restore.sh` and the maintenance
  runner all sourced it when present, so the indirection was already in place
  and the file had never existed on any install. Every script carried the path
  as a constant instead, which meant moving the repository was three edits
  that had to agree. It is written by `phase6_backup` and regenerated by
  `corex manage maintenance setup`, and a `BACKUP_ROOT` already in the file
  wins, because an operator who moved the repository must not have it moved
  back by a repair.

- **Prometheus scrapes itself.** Nothing anywhere could see
  `prometheus_tsdb_blocks_loaded` or
  `prometheus_tsdb_wal_truncations_failed_total`, which is how the fault below
  ran unnoticed while every outward sign was healthy.
...
### Fixed
- **Prometheus retained no history at all, while reporting itself ready and
  scraping correctly.** Three faults interlocked. An unclean shutdown
  corrupted WAL segment `00000211`, so checkpointing failed and
  `wal_truncations_total` stayed at 0 and the WAL grew to 13GB across 1,252
  segments. `--storage.tsdb.retention.size` is not a budget for blocks: in
  Prometheus 3.x `db.Size()` includes the WAL, so an 8GB figure was already
  spent before a single block counted, and 1,181 compactions produced 1,182
  immediate deletions and `blocks_loaded` of 0. The queryable window was under
  three hours on a 30 day retention setting. And because the WAL could not
  truncate, startup replayed it for 19 minutes 28 seconds, returning 503 to
  Grafana throughout, which reads as monitoring collecting nothing.

  The size figure now leaves real headroom above any plausible WAL, since
  unclean shutdowns are expected on this hardware rather than rare, the image
  is pinned off `:latest`, and Prometheus scrapes itself so the next
  occurrence is visible. Nothing outward-facing showed any of this: `/-/ready`
  answered 200 and `up` was 1 for every target the entire time.
